...
class CheckinController extends Controller
{
    ...
    public function getCourseCheckin(Request $request, $courseId)
    {
        ...
        $result = Cache::tags('checkin')->remember($courseId, Carbon::now()->addMinutes(60), function () use ($courseId, $course) {
            ...
            $c = array_column($notCheckedIn, 'class');
            $n = array_column($notCheckedIn, 'number');
            array_multisort($c, SORT_ASC, $n, SORT_ASC, $notCheckedIn);
            return array_merge($notCheckedIn, $checkedIn);
        });
